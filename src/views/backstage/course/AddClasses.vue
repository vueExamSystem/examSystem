<template>
	<section>
		<div class="panel">
			<div class="title">
				<span>选课班级列表（共{{totalClass}}个班级，{{totalExammer}}个学生生）</span>
                  ...
                  <!--   <el-table-column type="class" label="班主任" min-width="120">
                    ...
                    <el-table-column label="操作" min-width="220">
                        <template slot-scope="scope">
                            <el-button type="primary" size="small" @click="showStudentDialog(scope.row)">选择学生</el-button>
        ...
        <el-dialog title="添加学生" :visible.sync="studentDialogVisible" width="80%">
            <div class="panel">
...
<script>
    import {
        getCourseClassList,//获取参加考试的班级
        getUnCourseClassList,//获取学习该课程但未参加考试的班级
        addCourseClass, //添加考试班级
        delCourseClass, //删除考试班级
        getCourseStudentsByClassId, //获取班级学生
        saveStudents, //保存班级考生
        publishExam   //发布考试
    } from '../../../api/api';
	...
	export default {
		...
		methods: {
			getList(){//获取开始班级列表 不分页
				...
				getCourseClassList(para).then(res =>{ 
					this.rows = res.data;
            ...
            getUnExamClasses(){//根据考试科目获取学习该科目的班级-前端处理为树状关系数据
                ...
                var treeList = [];
                getUnCourseClassList(para).then(res => {
                    if(res.code!==0){
                    this.$message({
                        type: 'error',
                        message: res.msg
            ...
            getDialogStudents(id){
                ...
                this.studentLoading = true;
                this.studentRows = [];
                getCourseStudentsByClassId(para).then(res => {//不分页
            ...
            removeExamClass(row){//删除考试班级
                this.$confirm('删除后，该班级下所有学生将不能参加此课程学习，确认删除吗？', '提示', {}).then(() => {
                    let para = {
                        courseId: this.id, //考试id
                        termId:this.termId,
                        groupId: row.id //班级id
