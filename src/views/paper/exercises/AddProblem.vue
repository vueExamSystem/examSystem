...
<script>
	...
	import { getTrainProblemFilter, getTrainQuestionList, addTrainProblem } from '../../../api/api';
	...
	export default{
		...
		methods:{
            ...
            getFilter() {
                ...
                getTrainProblemFilter(para).then((res) => {
            ...
            search(){//数据不包含已存在试卷上的试题
                ...
                getTrainQuestionList(params).then(res => {
            ...
            addSave(){//添加选中(添加后试卷从初始化设为未启用？)
            	if(this.multipleSelection.length>0){
	            	...
	            	//console.log('addTrainProblemparams',params);
	            	params={
	            		json:JSON.stringify(params)
	            	}
	            	//to do
	            	addTrainProblem(params).then(res => {
	            		if(res.code == 0){
	            			this.$message({
	            				type: 'success',
	            				message: '添加成功'
