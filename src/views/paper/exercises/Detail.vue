...
<script>
	import { getTrainProblemList, updateTrainStatus, removeTrainProblem } from '../../../api/api';
	...
	export default{
		...
		methods:{
			init(){
				...
				getTrainProblemList(param).then(res => {
			...
			onSave(){//试卷保存
				...
				if(this.totalSet == this.info.totalPoint){
					...
					updateTrainStatus(params).then(res => {
			...
			removeProblem(id, blockType){
				...
				removeTrainProblem(params).then(res => {
					if(res.code == 0){
		                switch(blockType){
							case 'radio': 
