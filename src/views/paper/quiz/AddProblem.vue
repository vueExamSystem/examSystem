...
<script>
	...
	import { getTestProblemFilter, getQuizProblemList, addTestProblem } from '../../../api/api';
	...
	export default{
		...
		methods:{
            ...
            getFilter() {
                ...
                getTestProblemFilter(para).then((res) => {
            ...
            search(){//数据不包含已存在试卷上的试题
                ...
                getQuizProblemList(params).then(res => {
            ...
            addSave(){//添加选中(添加后试卷从初始化设为未启用？)
            	if(this.multipleSelection.length>0){
	            	...
	            	console.log('addTestProblemparams',params);
	            	params={
	            		json:JSON.stringify(params)
	            	}
	            	//to do
	            	addTestProblem(params).then(res => {
	            		if(res.code == 0){
	            			this.$message({
	            				type: 'success',
	            				message: '添加成功'
