...
<script>
	import { getTestProblemList, updateTestStatus, removeTestProblem } from '../../../api/api';
	...
	export default{
		...
		methods:{
			init(){
				...
				getTestProblemList(param).then(res => {
			...
			onSave(){//试卷保存
				...
				if(this.totalSet == this.info.totalPoint){
					...
					updateTestStatus(params).then(res => {
			...
			removeProblem(id, blockType){
				...
				removeTestProblem(params).then(res => {
					if(res.code == 0){
		                switch(blockType){
							case 'radio': 
